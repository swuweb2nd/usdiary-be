...
async function checkAndUpgradeTier(user) {
  ...
  // 사용자의 등급별 활동 확인 (일기 작성 횟수 등)
  const diaryCount = await Diary.count({ where: { user_id: userId, createdAt: { /* 이번 달 */ } } });
...
module.exports = { checkAndUpgradeTier };

// services/tierService.js
const { TierLog } = require('../models');

async function logTierChange(userId, previousTierId, newTierId) {
...
// 위의 checkAndUpgradeTier 함수에서 호출
if (user.tier_id !== previousTierId) {
  await logTierChange(userId, previousTierId, user.tier_id);
}
